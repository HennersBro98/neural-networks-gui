...
class NN(object):
    ...
    def FP(self,X,save_all=False):
        ...
        for i in range(1,self.noLayers):
            ### Forced to use less efficient form of matrix multiplication as normal isn't supported by PyQt
            stupid = matrixMul(self[i].edges*self[i].weights,a[i-1])
            a[i] = activation(stupid,self.activationKind)
            if self[i].bias:
    ...
    def BP(self,X,Y,lambdas=None):
        ...
        for l in range(-2,-(self.noLayers-1)-1,-1):                  #think about non-convolutional
            d[l] = matrixMul(d[l+1],self[l+1].weights)*a[l]*(1-a[l])        #only for sigmoid
            if self[l].bias:
                d[l] = np.delete(d[l],0,axis=1)
        D = [0]*(self.noLayers-1)
        for l in range(0,self.noLayers-1):
            D[l] = matrixMul(d[l].T,a[l]) 
